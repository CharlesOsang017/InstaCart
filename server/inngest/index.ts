...
const checkLowStock = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (
      !product ||
      product.stock === null ||
      product.stock >= LOW_STOCK_THRESHHOLD
    ) {
      return { skipped: true, stock: product?.stock };
    }
    await step.run("send-low0stock-email", async () => {
  ...
  async ({ event, step }) => {
    ...
    const result = await step.run("assign-rider", async () => {
      ...
      history.push({
        ...
        notes: `Auto-assigned to rider ${availableRider.name}`,
      });
